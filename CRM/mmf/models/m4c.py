...
class M4C(BaseModel):
    ...
    def _build_self_ocr_encoding(self):

        # OCR appearance feature: Faster R-CNN
        self.ocr_faster_rcnn_fc7_self = build_image_encoder(
            self._build_encoder_config(), direct_features=True
        )
        self.finetune_modules.append(
            {"module": self.ocr_faster_rcnn_fc7_self, "lr_scale": self.config.lr_scale_frcn}
        )

        self.linear_ocr_feat_to_mmt_in_self = nn.Linear(
            self.config.ocr.mmt_in_dim+300, self.mmt_config.hidden_size
        )

        # OCR location feature: relative bounding box coordinates (4-dim)
        self.linear_ocr_bbox_to_mmt_in_self = nn.Linear(4, self.mmt_config.hidden_size)

        self.ocr_feat_layer_norm_self = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ocr_bbox_layer_norm_self = nn.LayerNorm(self.mmt_config.hidden_size)
        self.ocr_drop_self = nn.Dropout(self.config.ocr.dropout_prob)

    def _build_mmt(self):
        self.m = MMT(self.mmt_config)
        self.mmt = self.m

        #change
        self.related_classifier=nn.Sequential(nn.Linear(768, 1),nn.Sigmoid())
    ...
    def forward(self, sample_list):
        ...
        self._forward_self_ocr_encoding(sample_list, fwd_results)
        self._forward_mmt_and_output(sample_list, fwd_results)

        # only keep scores in the forward pass results
        if self.training:
            results = {"loss_related": fwd_results["loss_related"]}
        else:
            results = {"loss_related": fwd_results["loss_related"],
                       "pred_related_score": fwd_results["pred_related_score"]}
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        ...
        obj_mmt_in = self.obj_feat_layer_norm(self.linear_obj_feat_to_mmt_in(obj_feat)) + \
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results):
        ...
        ocr_fasttext = sample_list.context
    ...
    def _forward_self_ocr_encoding(self, sample_list, fwd_results):
        # OCR FastText feature (300-dim)
        ocr_fasttext = sample_list.self_context
        ocr_fasttext = F.normalize(ocr_fasttext, dim=-1)
        assert ocr_fasttext.size(-1) == 300

        ocr_encoder_feats = sample_list.self_context_encoder_feats
        ocr_encoder_feats = F.normalize(ocr_encoder_feats, dim=-1)
        assert ocr_encoder_feats.size(-1) == 300

        # OCR PHOC feature (604-dim)
        ocr_phoc = sample_list.self_context_feature_1
        ocr_phoc = F.normalize(ocr_phoc, dim=-1)
        assert ocr_phoc.size(-1) == 604

        # OCR appearance feature: Faster R-CNN fc7
        ocr_fc6 = sample_list.self_image_feature_1#(bs,2048)
        ocr_fc7 = self.ocr_faster_rcnn_fc7_self(ocr_fc6)
        ocr_fc7 = F.normalize(ocr_fc7, dim=-1)

        # OCR order vectors (legacy from LoRRA model; set to all zeros)
        # TODO remove OCR order vectors; they are not needed
        ocr_order_vectors = torch.zeros_like(sample_list.self_order_vectors)

        #change
        ocr_feat = torch.cat(
            [ocr_fasttext, ocr_encoder_feats,ocr_phoc, ocr_fc7, ocr_order_vectors], dim=-1
        )
        ocr_bbox = sample_list.self_ocr_bbox_coordinates

        ocr_mmt_in = self.ocr_feat_layer_norm_self(
            self.linear_ocr_feat_to_mmt_in_self(ocr_feat)
        ) + self.ocr_bbox_layer_norm_self(self.linear_ocr_bbox_to_mmt_in_self(ocr_bbox))

        ocr_mmt_in = self.ocr_drop_self(ocr_mmt_in)
        ocr_mmt_in=ocr_mmt_in.view(-1,1,768)
        fwd_results["ocr_self_mmt_in"] = ocr_mmt_in

        # binary mask of valid OCR vs padding
        ocr_nums = sample_list.self_context_info_0.max_features
        fwd_results["ocr_self_mask"] = _get_mask(ocr_nums, ocr_mmt_in.size(1))
    ...
    def _forward_mmt(self, sample_list, fwd_results):
        ...
        mmt_results = self.mmt(
            ...
            ocr_self_emb=fwd_results["ocr_self_mmt_in"],
            ocr_self_mask=fwd_results["ocr_self_mask"],
    ...
    def _forward_mmt_and_output(self, sample_list, fwd_results):
        if self.training:
            ...
            mmt_ocr_self_output = fwd_results["mmt_ocr_self_output"]
            ocr_mask = fwd_results["ocr_mask"]
            ocr_self_mask = fwd_results["ocr_self_mask"]

            mmt_ocr_self_output_2 = mmt_ocr_self_output.view(-1, 768)
            mmt_ocr_self_related_score=self.related_classifier(mmt_ocr_self_output_2)
            related_label=torch.FloatTensor(sample_list.related_label).unsqueeze(-1).to(mmt_ocr_self_related_score.device)
            fwd_results['loss_related']=self.criterion(mmt_ocr_self_related_score,related_label)
            ...
            related_label = torch.FloatTensor(sample_list.related_label).unsqueeze(-1).to(mmt_ocr_self_related_score.device)
            fwd_results['loss_related'] = self.criterion(mmt_ocr_self_related_score, related_label)
            fwd_results['pred_related_score'] = mmt_ocr_self_related_score
...
class MMT(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoder_inputs = torch.cat([txt_emb, obj_emb, ocr_emb, ocr_self_emb], dim=1)
        attention_mask = torch.cat([txt_mask, obj_mask, ocr_mask, ocr_self_mask], dim=1)

        # offsets of each modality in the joint embedding space
        txt_max_num = txt_mask.size(-1)
        obj_max_num = obj_mask.size(-1)
        ocr_max_num = ocr_mask.size(-1)
        ocr_self_max_num = ocr_self_mask.size(-1)#1
        txt_begin = 0
        txt_end = txt_begin + txt_max_num
        ocr_begin = txt_max_num + obj_max_num
        ocr_end = ocr_begin + ocr_max_num
        ocr_self_end=ocr_end+ocr_self_max_num
        ...
        mmt_ocr_self_output = mmt_seq_output[:, -ocr_self_max_num:]

        results = {
            "mmt_seq_output": mmt_seq_output,
            "mmt_txt_output": mmt_txt_output,
            "mmt_ocr_output": mmt_ocr_output,
            "mmt_ocr_self_output": mmt_ocr_self_output,
        }
        return results
